                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.blogs.update", [$blog->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $blog->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('slug') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="slug" id="slug" value="{{ old('slug', $blog->slug) }}" required>
                            @if($errors->has('slug'))
                        ...
                        <div class="form-group {{ $errors->has('is_featured') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ $blog->is_featured || old('is_featured', 0) === 1 ? 'checked' : '' }}>
                                <label for="is_featured" style="font-weight: 400">{{ trans('cruds.blog.fields.is_featured') }}</label>
                            </div>
                            @if($errors->has('is_featured'))
                                <span class="help-block" role="alert">{{ $errors->first('is_featured') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.blog.fields.is_featured_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('is_premium') ? 'has-error' : '' }}">
                            <div>
                                <input type="hidden" name="is_premium" value="0">
                                <input type="checkbox" name="is_premium" id="is_premium" value="1" {{ $blog->is_premium || old('is_premium', 0) === 1 ? 'checked' : '' }}>
                                <label for="is_premium" style="font-weight: 400">{{ trans('cruds.blog.fields.is_premium') }}</label>
                            </div>
                            @if($errors->has('is_premium'))
                                <span class="help-block" role="alert">{{ $errors->first('is_premium') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.blog.fields.is_premium_helper') }}</span>
                        </div>
                        ...
                        <div class="form-group {{ $errors->has('blog_categories') ? 'has-error' : '' }}">
                            <label for="blog_categories_id">{{ trans('cruds.blog.fields.blog_categories') }}</label>
                            <select class="form-control select2" name="blog_categories_id" id="blog_categories_id">
                                @foreach($blog_categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('blog_categories_id') ? old('blog_categories_id') : $blog->blog_categories->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('blog_categories'))
                                <span class="help-block" role="alert">{{ $errors->first('blog_categories') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.blog.fields.blog_categories_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('blog_tags') ? 'has-error' : '' }}">
                            <label for="blog_tags_id">{{ trans('cruds.blog.fields.blog_tags') }}</label>
                            <select class="form-control select2" name="blog_tags_id" id="blog_tags_id">
                                @foreach($blog_tags as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('blog_tags_id') ? old('blog_tags_id') : $blog->blog_tags->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('blog_tags'))
                                <span class="help-block" role="alert">{{ $errors->first('blog_tags') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.blog.fields.blog_tags_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('post_date') ? 'has-error' : '' }}">
                            <label class="required" for="post_date">{{ trans('cruds.blog.fields.post_date') }}</label>
                            <input class="form-control datetime" type="text" name="post_date" id="post_date" value="{{ old('post_date', $blog->post_date) }}" required>
                            @if($errors->has('post_date'))
                                <span class="help-block" role="alert">{{ $errors->first('post_date') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.blog.fields.post_date_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('picture_image') ? 'has-error' : '' }}">
                            <label for="picture_image">{{ trans('cruds.blog.fields.picture_image') }}</label>
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.blog.fields.picture_image_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('gallery_images') ? 'has-error' : '' }}">
                            <label for="gallery_images">{{ trans('cruds.blog.fields.gallery_images') }}</label>
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.blog.fields.gallery_images_helper') }}</span>
                        </div>
                        ...
                        <div class="form-group {{ $errors->has('meta_description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="meta_description" id="meta_description">{{ old('meta_description', $blog->meta_description) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('meta_keywords') ? 'has-error' : '' }}">
                            <label for="meta_keywords">{{ trans('cruds.blog.fields.meta_keywords') }}</label>
                            <textarea class="form-control" name="meta_keywords" id="meta_keywords">{{ old('meta_keywords', $blog->meta_keywords) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('template') ? 'has-error' : '' }}">
                            <label for="template_id">{{ trans('cruds.blog.fields.template') }}</label>
                            <select class="form-control select2" name="template_id" id="template_id">
                                @foreach($templates as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('template_id') ? old('template_id') : $blog->template->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('template'))
                                <span class="help-block" role="alert">{{ $errors->first('template') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.blog.fields.template_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('format') ? 'has-error' : '' }}">
                            <label>{{ trans('cruds.blog.fields.format') }}</label>
                            <select class="form-control" name="format" id="format">
                                <option value disabled {{ old('format', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                                @foreach(App\Models\Blog::FORMAT_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('format', $blog->format) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('format'))
                                <span class="help-block" role="alert">{{ $errors->first('format') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.blog.fields.format_helper') }}</span>
                        </div>
...
<script>
    ...
    init: function () {
@if(isset($blog) && $blog->picture_image)
      var file = {!! json_encode($blog->picture_image) !!}
...
@if(isset($blog) && $blog->gallery_images)
      var files = {!! json_encode($blog->gallery_images) !!}
          for (var i in files) {
          var file = files[i]
          this.options.addedfile.call(this, file)
          this.options.thumbnail.call(this, file, file.preview)
