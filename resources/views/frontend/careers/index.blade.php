        ...
        <div class="col-md-12">
            @can('career_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.careers.create') }}">
                            ...
                            <tbody>
                                @foreach($careers as $key => $career)
                                    <tr data-entry-id="{{ $career->id }}">
                                        ...
                                        <td>
                                            @can('career_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.careers.show', $career->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('career_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.careers.edit', $career->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('career_delete')
                                                <form action="{{ route('frontend.careers.destroy', $career->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('career_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.careers.massDestroy') }}",
