        ...
        <div class="col-md-12">
            @can('permalink_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.permalinks.create') }}">
                            ...
                            <tbody>
                                @foreach($permalinks as $key => $permalink)
                                    <tr data-entry-id="{{ $permalink->id }}">
                                        ...
                                        <td>
                                            @can('permalink_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.permalinks.show', $permalink->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('permalink_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.permalinks.edit', $permalink->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('permalink_delete')
                                                <form action="{{ route('frontend.permalinks.destroy', $permalink->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('permalink_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.permalinks.massDestroy') }}",
